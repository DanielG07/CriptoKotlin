...
class CriptosRepositoryImpl @Inject constructor(private val remoteDataSource: CriptoRemoteDataSource, private val localDataSource: CriptoLocalDataSource) :
    CriptosRepository {

    override fun getCriptos(refresh: Boolean): Flow<DataState<List<Cripto>, ErrorResponse>> =

        when (refresh) {
            true -> // Va a la red e inserta una tabla
                flow {
